...
################## Auto generated ##################
[databases]
${DB_NAME:-master} = host=${DB_HOST:?"Setup pgbouncer config error! You must set DB_HOST env"} \
port=${DB_PORT:-5432} user=${DB_USER:-postgres}
${CLIENT_ENCODING:+client_encoding = ${CLIENT_ENCODING}\n}\

${RDB_NAME:-rreplica} = host=${RDB_HOST:?"Setup pgbouncer config error! You must set RDB_HOST env"} \
port=${DB_PORT:-5432} user=${DB_USER:-postgres}
${CLIENT_ENCODING:+client_encoding = ${CLIENT_ENCODING}\n}\
...
## this is for pg-bouncer-rr
routing_rules_py_module_file = ${ROUTING_RULE_PATH:-${PG_CONFIG_DIR}/routing_rules.py}
rewrite_query_py_module_file = ${REWRITE_QUERY_PATH:-${PG_CONFIG_DIR}/rewrite_query.py}
